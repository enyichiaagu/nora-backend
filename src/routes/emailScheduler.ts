...
router.post('/', async (req, res) => {
  try {
    const { email, scheduledTime, subject, message }: ScheduleEmailRequest = req.body;
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        subject: subject || 'Scheduled Reminder from NORA',
        message: message || 'This is your scheduled reminder.',
        status: 'pending',
      })
      .select()
      .single();
